...
class AntStateDelivering(State):
    ...
    def check_conditions(self):
        if Vector2(*NEST_POSITION).get_distance_to(self.ant.location)<NEST_SIZE:
            if(randint(1,10)==1):
                self.ant.drop(self.ant.world.backgroud)
                return "exploring"
        return None
    def entry_actions(self):
        self.ant.speed=60
...
class World(object):
    def __init__(self):
        self.entities = {}
        self.entity_id = 0
        self.backgroud = pygame.surface.Surface(SCREEN_SIZE).convert()
        self.backgroud.fill((255, 255, 255))
        pygame.draw.circle(self.backgroud, (200, 255, 200), NEST_POSITION, int(NEST_SIZE))
